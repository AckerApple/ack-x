import { method as xDate } from "./date";
var ackYear = (function () {
    ...
    ackYear.prototype.setStartDate = function (date) {
        var isObject = typeof (date) == 'object', isYearString = !isObject && !isNaN(Number(date)), isYear = isYearString || (!xDate(date).isDate() && !isNaN(date));
        if (isYear) {
    ...
    ackYear.prototype.getDateOfLastWeekday = function () {
        var d = this.getStartDate(), addAmount = -xDate(d).dayOfWeek() + 6, dateA = new Date(d.setDate(d.getDate() + addAmount));
