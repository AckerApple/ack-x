"use strict";
// var ack = global.ack
const AckDate = require('../../js/date').AckDate;
...
describe('ack.date',function(){
	...
	it.only('#diffStats',function(){
		...
		assert.equal(dateStats.days, 1, '36-days')
		assert.equal(dateStats.hours, 0, '36-hours')
		...
		assert.equal(dateStats.days, 0, '31-days')
		assert.equal(dateStats.hours, 23, '31-hours')
		...
		start = ack.date('08/04/2020').gotoSod()
		diffDay = ack.date('08/01/2020').gotoSod()
		dateStats = start.diffStats( diffDay )
		assert.equal(dateStats.years, 0, '0-years')
		assert.equal(dateStats.months, 4, '0-months')
		assert.equal(dateStats.weeks, 3, '0-weeks')
		assert.equal(dateStats.days, 3, '3-days')
		assert.equal(dateStats.hours, 0, '0-hours')
		assert.equal(dateStats.minutes, 0, '0-minutes')
		assert.equal(dateStats.seconds, 0, '0-seconds')
